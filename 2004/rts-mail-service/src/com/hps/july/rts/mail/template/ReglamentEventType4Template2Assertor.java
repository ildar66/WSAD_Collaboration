/*
 *  $Id: ReglamentEventType4Template2Assertor.java,v 1.1 2007/04/29 17:22:05 vglushkov Exp $
...
/**
 * Темплейт письма для события 4
 * оповещения Утверждающего ежедневно о
 * необходимости утверждения
 * Консолидированной заявки (
 * начиная со следующего рабочего дня после прихода заявки)
 * Subject: Заявка ТС  не утверждена в течении ...
 * To: 	КМ
 * @author <a href="mailto:vadim.glushkov@sntru.com">Vadim Glushkov</a>
 * @version $Revision: 1.1 $ $Date: 2007/04/29 17:22:05 $
 */
public class ReglamentEventType4Template2Assertor extends RTSMailTemplate {

	private long delay;
	protected ConsolidatedRequest request;
	protected StringBuffer reference;

	public ReglamentEventType4Template2Assertor(ConsolidatedRequest request, long createTime, String host) {
		this.request = request;
		delay = (int)((System.currentTimeMillis() - createTime)/(1000L * 60L * 60L * 24L));
		this.reference = new StringBuffer(host);
		this.reference.append("/rts/ShowConsolidatedRequestsToConfirm.do?navclear=1&currentRole=KM&itemMenu=item1_17_5_13&isNumber=on&number=").append(request.getNumber());
	}
	...
	public String getSubject() {
		...
		msg.append(" не утверждена в течении ");
		msg.append(delay);
		msg.append(".");
		return msg.toString();
	}
