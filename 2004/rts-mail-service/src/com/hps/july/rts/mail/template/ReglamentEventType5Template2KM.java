/*
 *  $Id: ReglamentEventType5Template2KM.java,v 1.1 2007/04/29 17:22:04 vglushkov Exp $
...
/**
 * RFC 060727
 * Темплейт письма для события 5
 *  оповещения Исполнителей в течение последних 3-х дней
 * до истечения планируемого срока о необходимости исполнения
 *  или переноса срока исполнения Элементарной заявки
 * Subject: Заявка ТС  не выполена в течении ( или перенести дату исполенния) в течении ...
 * To: 	КМ
 * @author <a href="mailto:vadim.glushkov@sntru.com">Vadim Glushkov</a>
 * @version $Revision: 1.1 $ $Date: 2007/04/29 17:22:04 $
 */
public class ReglamentEventType5Template2KM extends RTSMailTemplate {

	private long delay;
	protected ElementaryRequest request;
	protected ConsolidatedRequest cRequest;
	protected StringBuffer reference;

	public ReglamentEventType5Template2KM(ElementaryRequest request, ConsolidatedRequest cRequest, long createTime, String host) {
		this.request = request;
		this.cRequest = cRequest;
		delay = (int)((System.currentTimeMillis() - createTime)/(1000L * 60L * 60L * 24L));
		this.reference = new StringBuffer(host);
		this.reference.append("/rts/ConsolidatedRequestsListAction.do?navclear=1&currentRole=KM&itemMenu=item1_17_5_11&isNumber=on&number=").append(cRequest.getNumber());
	}
	...
	public String getSubject() {
		...
		msg.append(" не выполена в течении ( или перенести дату исполенния)");
		msg.append(delay);
		msg.append(".");
		return msg.toString();
	}
