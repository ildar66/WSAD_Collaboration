...
import com.hps.july.terrabyte.imp.command.BSProjectCommand;
 ...
 * Time: 17:07:10
 */
public class TestBSProjectFilesLoaderProcessor {
	...
	public void test() {
			...
			try {
				con = getConnection();
				BSProjectCommand command = new BSProjectCommand(new Integer(345), con, log);
				TerrabyteLoaderProcessor.executeLoaderCommand(command, "websphere", new Integer(9080));
				//TerrabyteLoaderProcessor.executeLoaderCommand(command, "nri-dev-21", new Integer(8090));
...
*/
			...
			connection = DriverManager.getConnection("jdbc:informix-sqli://172.21.9.131:1541:informixserver=beeinf_app;database=july;DB_LOCALE=ru_RU.1251;CLIENT_LOCALE=ru_RU.1251;user=informix;password=beeinf01;informixLockModeWait=60");
			connection.setTransactionIsolation(Connection.TRANSACTION_READ_COMMITTED);
		} catch(Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		TestBSProjectFilesLoaderProcessor test = new TestBSProjectFilesLoaderProcessor();
		test.test();
