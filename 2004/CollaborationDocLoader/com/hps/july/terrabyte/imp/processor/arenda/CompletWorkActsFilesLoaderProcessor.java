/*
 *  $Id: CompletWorkActsFilesLoaderProcessor.java,v 1.2 2007/06/15 17:12:50 nizhikov Exp $
...
import com.hps.july.terrabyte.imp.helper.arenda.CompletWorkActsFilesHelper;
 ...
 * Time: 10:58:30
 */
public class CompletWorkActsFilesLoaderProcessor extends AbstractFileTreeLoaderProcessor {

    public CompletWorkActsFilesLoaderProcessor(String hostName, Integer port) throws TerrabyteExternalClientException {
        ...
        appLog = new AppLog("CompletWorkActsFilesLoaderProcessor");
    }
    ...
    public void execute(Command command) throws LoaderException, BaseException {
        ...
        try {
            Collection fileList = CompletWorkActsFilesHelper.getFilesInfo(command.getConnection());
            System.out.println(" fileList count ["+fileList.size()+"] ");
            for (Iterator i1 = fileList.iterator(); i1.hasNext();) {
                CompletWorkActsFileInfo fileInfo = (CompletWorkActsFileInfo) i1.next();
                System.out.println("Documents ["+fileInfo.getIdHeader()+"] ["+fileInfo.getName()+"] ");
                InputStream body = null;
                try {
                    body = CompletWorkActsFilesHelper.getFileBody(command.getConnection(), fileInfo);
...
//                    fos.close();
                    ...
                    CompletWorkActsFilesHelper.setExtendedParameters(command.getConnection(), fileInfo, fileInfo.getIdent());
    ...
    protected void processFile(CompletWorkActsFileInfo info, InputStream fis) {
        try {
            addFileProceed();
            ...
            if(document.getDocumentName() == null) document.setDocumentName("cwaWord.doc");
            document.setData(fis);
            Document newDoc = extClient.addObjectDocument(document, info.getIdHeader(), ObjectTypes.COMPLET_WORK_ACTS, "vad");
