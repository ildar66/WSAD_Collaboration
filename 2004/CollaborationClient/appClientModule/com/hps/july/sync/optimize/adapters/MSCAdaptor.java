 ...
 * Адаптер для переноса коммутаторов.
 */
public class MSCAdaptor extends DefaultCollaboration {
    ...
    private void prepareSelect() {
		if (preparedSelectSourceDb == null) {
			...
			buf.append("switches w, equipment e, storageplaces s, outer (resources r, organizations o) ");
	...
	protected String getSqlChangeInSource() {
		...
		result.append("switches w, equipment e, storageplaces s, outer (resources r, organizations o) ");
    ...
    public MSCAdaptor(DB argConOptimize, DB argConNRI) {
        super(argConOptimize, argConNRI, "nrimsc", "switches", "s.modified", new DetColumnMap(), argConNRI);
    }
