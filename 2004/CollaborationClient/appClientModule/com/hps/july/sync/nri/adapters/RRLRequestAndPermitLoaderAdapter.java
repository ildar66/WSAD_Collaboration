...
import com.hps.july.terrabyte.imp.command.status.RRLRequestAndPermitCommand;
...
/**
 * Адаптер для загрузки общей информации по разрешениям и завякам по РРЛ
 * от Львова Сергея
 * Author: Vadim Glushkov (mailto: v.glushkov@mail.ru)
 * Date: 03.04.2006
 * Time: 19:03:59
 */
public class RRLRequestAndPermitLoaderAdapter extends DefaultCollaboration {
     ...
     * @param prop
     */
    public RRLRequestAndPermitLoaderAdapter(DB targerDB, DB logDB, Properties prop) {
        super(targerDB, null, "_LOADRRLRequestAndPermit", null, null, null, logDB);
        this.prop = prop;
    }
    /* (non-Javadoc)
    ...
    protected boolean doBeforeTask(Query qry) {
        ...
        String imageDirPath = prop.getProperty("DirPath.RRLRequestAndPermit");
        if(imageDirPath == null) {
            QueryProcessing.addLogMessage(getLogDbConnection(), qry, QueryProcessing.MSG_ERROR, "Cannot find property 'DirPath.RRLRequestAndPermit' for loader !");
            return false;
        }
        File imageDirPathFile = new File(imageDirPath);
        if(!imageDirPathFile.exists()) {
            QueryProcessing.addLogMessage(getLogDbConnection(), qry, QueryProcessing.MSG_ERROR, "Cannot find path to request loader !");
            return false;
        }

        try {
            System.out.println("Вызов сторонней процедуры TerrabyteSanPassportLoaderAdapter :");
            RRLRequestAndPermitCommand command = new RRLRequestAndPermitCommand(new Integer(qry.getQueryId()), getTargetDbConnection(), getLogDbConnection());
