...
<%}%>						</TD>
		    ...
		    <bean:define id="objectId" name="listcheckbox" property="<%=columnField%>"/>
					...
					<%
						...
						{
							String hRef = cellItem.getLink()+ "&objectId=" +  objectId  + "&refId="+refId + "&contextId=" + contextId + "&resourceUri="+resourceUri + "&perspective=" + perspective;
					%>
							<A HREF="<%=hRef%>">
				     <% } 
...
<A target="statuswindow" href="/admin/status?text=true&type=node&node=<%=name%>"><IMG name="statusIcon" border="0" SRC="/admin/status?type=node&node=<%=name%>" ALT="<%=statusAlt%>"></A>
